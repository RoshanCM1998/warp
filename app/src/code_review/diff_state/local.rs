...
use super::{
    ...
    GitFileStatus, GitOpResult, StagingSection,
};

/// Maximum number of files whose diff + base content are loaded concurrently
/// during a full diff load. Bounds the number of in-flight `git` subprocesses
/// so a large change set doesn't spawn hundreds of processes at once, while
/// still parallelizing enough to hide per-process spawn latency — the dominant
/// cost on Windows, where serial `git diff`/`git show` spawns made the panel
/// sit in a loading state for seconds on larger diffs.
...
impl LocalDiffStateModel {
    ...
    async fn diff_state_against_head(repo_path: &Path) -> Result<GitDiffWithBaseContent> {
        ...
        // Load each file's diff + base content concurrently (bounded) rather than
        // one-at-a-time. Each file costs two `git` subprocess spawns
        // (`git diff` + `git show HEAD:<file>`); serialized, that dominated the
        // load time on large diffs. `buffered` preserves input order so the
        // sidebar ordering stays deterministic.
        let files: Vec<FileDiffAndContent> = futures::stream::iter(changed_files)
            .map(|(file_path, status)| {
                ...
                async move {
                    ...
                    Ok::<FileDiffAndContent, anyhow::Error>(FileDiffAndContent {
                        file_diff,
                        content_at_head,
                    })
                }
            })
            .buffered(MAX_CONCURRENT_FILE_LOADS)
            .try_collect()
            .await?;

        let total_additions = files.iter().map(|f| f.file_diff.additions()).sum();
        let total_deletions = files.iter().map(|f| f.file_diff.deletions()).sum();
    ...
    async fn diff_state_against_head_split(repo_path: &Path) -> Result<GitDiffWithBaseContent> {
        ...
        // Flatten both sections into one ordered list (staged first, then
        // unstaged) so the per-file loads can run concurrently while `buffered`
        // preserves that ordering. See [`Self::diff_state_against_head`] for why
        // this is parallelized rather than awaited one file at a time.
        let entries: Vec<(StagingSection, String, GitFileStatus)> = staged
            .into_iter()
            .map(|(path, status)| (StagingSection::Staged, path, status))
            .chain(
                unstaged
        ...
        let files: Vec<FileDiffAndContent> = futures::stream::iter(entries)
            .map(|(section, file_path, status)| {
                let is_binary = binary_files.contains(&file_path);
                async move {
                    let mut file_diff = Self::get_file_diff_for_section(
                        repo_path, &file_path, &status, is_binary, section,
