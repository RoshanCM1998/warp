        ...
        CommitSubAction::CycleIntent => {
            if let GitDialogMode::Commit(state) = me.mode_mut() {
                ...
                state.intent = match state.intent {
                    CommitChainMode::CommitOnly => CommitChainMode::CommitAndPush,
                    CommitChainMode::CommitAndPush if pr_available => {
                        CommitChainMode::CommitAndCreatePr
                    }
                    CommitChainMode::CommitAndPush => CommitChainMode::CommitOnly,
                    CommitChainMode::CommitAndCreatePr => CommitChainMode::CommitOnly,
                };
            }
            if let GitDialogMode::Commit(state) = me.mode() {
                apply_intent_selector(state, ctx);
